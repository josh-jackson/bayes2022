...
---
Assumes that fitted values represent values from possible distributions. 
```{r}
MR %>%
...
---
## MLM priors
This new parameter represents the possible range of deviations from the average
```{r, echo = FALSE, message=FALSE, warning=FALSE}
...
mr.4 %>% 
 spread_draws(b_iv1, b_iv2,b_iv3) %>% 
  mutate(b_1 = b_iv1-mean(b_iv1),
         b_2 = b_iv2-mean(b_iv2),
         b_3 = b_iv3-mean(b_iv3)) %>% 
  pivot_longer(b_1:b_3) %>% 
...
We get greater uncertainty and a greater push towards similar values across the groups. Why? (1) our regularizing (centered around zero) priors push mean to zero. (2) we are partially pooling values across groups, shrinking the mean to the group mean. 

(3) Incorporating uncertainty in the random effects. The SD parameter describes how much uncertainty in the differences between the groups. We said that they could differ, but they didn't. The model is taking the potential that they differ and placing that into the model as uncertainty. Now there is larger dispersion within each of the posteriors for each group. This is because we said these differences "could happen"  
...
MR %>%
  ...
  add_fitted_draws(mr.11, n = 100) %>%
  ggplot(aes(x = SS_c, y = happiness, color = iv.d)) +
  geom_line(aes(y = .value, group = paste(iv.d, .draw)),alpha = .1) +
  geom_point(data = MR) 
```
...
MR %>% 
data_grid(SS_c = seq_range(SS_c, n = 20), FQ_c = c(-2.5,0,2.5), .model = MR) %>%
  add_fitted_draws(mr.13) %>% 
  ggplot(aes(x = SS_c, y = happiness, group = FQ_c %>% factor()))+
  stat_lineribbon(aes(y = .value, color = FQ_c %>% factor(), alpha = .05 ), show.legend = FALSE) +
  geom_point(data = MR) 
...
data_grid(SS_c = seq_range(SS_c, n = 20), FQ_c = c(-2.5,0,2.5), .model = MR) %>%
  ...
  stat_lineribbon(aes(y = .value, color = FQ_c %>% as.character(), alpha = .05 ), show.legend = FALSE) +
  geom_point(data = MR) 
```
...
MR %>% 
data_grid(SS_c = seq_range(SS_c, n = 20), FQ_c = c(-2.5,0,2.5), .model = MR) %>%
  add_fitted_draws(mr.13, n = 50)  %>%
  ggplot(aes(x = SS_c, y = happiness, group = interaction(SS_c, FQ_c), color = FQ_c %>%  as.character())) +
  geom_line(aes(y = .value, group = paste(FQ_c, .draw), alpha = .5),show.legend = FALSE) +  
...
Note that there are no SS derivations, no Fs -- because we don't use that type of estimation! 

These are just linear models, so what are we doing different? 
