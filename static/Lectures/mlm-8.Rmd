...
L1
$$Y_{t i} = \beta_{0i} + \beta_{1i}Study_{ti} + \varepsilon_{ti}$$
      
L2
$$\beta_{0i} = \gamma_{00} +\gamma_{01}Age_i+ U_{0i}$$
$$\beta_{1i} = \gamma_{10} +\gamma_{11}Age_i+ U_{1i}$$
...
$$\beta_{0i} = \gamma_{00} +\gamma_{01}Age_i+ U_{0i}$$
$$\beta_{1i} = \gamma_{10} +\gamma_{11}Age_i+ U_{1i}$$
There is some average effect ( $\gamma_{00}$ ; fixed effect) that people vary around like an intercept. You can put in predictors (e.g., Age) and there is person specific residual ( $U_{0i}$ ). 

---
### Residuals and random effects 

L1: $$Y_{t i} = \beta_{0i} + \beta_{1i}Study_{ti} + \varepsilon_{ti}$$
      
L2: $$\beta_{0i} = \gamma_{00} +\gamma_{01}Age_i+ U_{0i}$$
$$\beta_{1i} = \gamma_{10} +\gamma_{11}Age_i+ U_{1i}$$

Together a person's intercept ( $\beta_{0i}$ ) or slope ( $\beta_{1i}$ ) is made up of the fixed effect, any regression coefficients and $U_{0i}$ and $U_{1i}$ reflect deviations from the fixed effect (plus any predictors). 

Josh's test score $\beta_{0i} = 10$ = avg score ( $\gamma_{00} = 5$ ) + age effect ( $\gamma_{01} = -2$ ) + random effect ( $U_{0i} = 7$ )
...
L1  
$$Y_{t i} = \beta_{0i} +  \varepsilon_{ti}$$
      
L2
$$\beta_{0i} = \gamma_{00} + U_{0i}$$
Combined 
$$Y_{t i} =  \gamma_{00} + U_{0i} +  \varepsilon_{ti}$$
...
Combined 
$$Y_{t i} =  \gamma_{00} +  \gamma_{10}Study_{ti} + U_{0i} + U_{1i}Study_{ti} + \varepsilon_{ti}$$
...
For MLMs we will be breaking up unexplained variance ( $\varepsilon$ ) into multiple buckets ( $U_0$ + $U_1$ + $e_{ti}$ and potentially more).
...
L1
$$Y_{ti} = \beta_{0i} +  \varepsilon_{ti}$$
L2
$$\beta_{0} = \gamma_{00} + U_{0i}$$
 
Where does $U_{0i}$ come from? If we calculated each by hand, through taking the average test score for each person i and subtracting that from the grand mean test score, would that equal $U_{0i}$ ?
 
---
## Complete, partial and no pooling

Complete pooling assumes everyone is the same, with $U_{0i}$ being zero for everyone. $Y_{t i} =  \gamma_{00} +  \varepsilon_{ti}$ 
...
How do we figure out the amount of pooling? The extent of shrinkage depends on a few things (eg number of observations per group) but also depends on our priors. We are going to learn the prior based on the data. This is known as a hyperprior
...
The population is larger than what we have. This is true for almost anything we do. We sample paradigms, we sample people, we usually never have a full population under our study. 
