...
.pull-left[ The extent of shrinkage depends on a few things (eg number of observations per group) but also depends on our priors. This is known as a hyperprior
]
...
If you think about parameters you are estimating the number of priors are simple. But in terms of what those priors represent, they are providing a prior on a prior. The intercept and the SD are both providing estimates of means. We could directly 
...
posterior_samples(mlm.id) %>% 
  pivot_longer(b_ID6:b_ID229, names_to="ID") %>% 
...
SD of random effect can turn on/off shrinkage. Setting SD to zero says everyone is the same. Infinite SD says all groups are unrelated (no pooling). In the middle gives you partial pooling. Remember, with MLE, you are using flat priors.   
