...
---
Same information, but an easier way to handle all this information.
```{r}
library(tidybayes)
tidy_draws(m.1)
```
...
## fitted values
.pull-left[
TWe can calculate not only the mean but also the dispersion. In lm land we had to use a funky equation to calculate the CI around some predicted value of X. Now we can use samples. It is just counting up where the xx% of samples fall. 
...
Our posterior had 1000 rows (samples). So this is taking each person and feeding them into the 1000 different regression equations and spitting out a fitted/expected/predicted value. 
...
.pull-left[If a model is a good fit we should be able to use it to generate data that resemble the data we observed. 
