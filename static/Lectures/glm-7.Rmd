...
When we want to create fitted values we run into an extra complciation. Do we want to plot odds (original metric) or logits (our transformed metric)? We haven't had to deal with this prior because we didnt do any transformations with a gaussian normal. 
...
Anytime people have more than two choices in some task. Trying to understand different demographic categories (race/ethnicity/gender identitites/etc). 
